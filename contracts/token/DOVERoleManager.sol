 ...
 * @dev Manages role-based access control with multi-signature approval
 */
abstract contract DOVERoleManager is DOVEBase {
    // ================ Events ================
    ...
    function approveRoleOperation(bytes32 operation) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // Extract role and account from operation
        // This is more complex in practice and would require storing additional mapping
        // For simplicity, we use the operation ID directly
    ...
    function _processRoleChangeApproval(bytes32 role, address account, bytes32 operation) internal returns (bool) {
        ...
        if (_roleChangeInitiatedTime[operation] == 0) {
            ...
            // Check for operation timeout
            require(block.timestamp < _roleChangeInitiatedTime[operation] + ROLE_APPROVAL_EXPIRATION, 
                "Operation expired, please start again");
        }
