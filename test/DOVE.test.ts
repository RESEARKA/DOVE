...
import { DOVEAdmin } from '../typechain-types';
...
describe('DOVE Token Ecosystem', function () {
  ...
  async function deployTokenWithOptions(autoLaunch = true) {
    ...
    await doveAdmin.connect(admin).grantRole(await doveAdmin.DEFAULT_ADMIN_ROLE(), await doveDeployer.getAddress());
    ...
    const deployEvent = receipt?.logs.find(
      (log) => log.topics[0] === ethers.id('DOVEEcosystemDeployed(address,address,address,address,address)')
    );
    ...
    await doveAdmin.connect(admin).revokeRole(await doveAdmin.DEFAULT_ADMIN_ROLE(), await doveDeployer.getAddress());
    ...
    const _adminContractAddress = await doveGovernance.getAdminContract();

    // Set up roles on DOVEAdmin
    await doveAdmin.connect(admin).grantRole(await doveAdmin.FEE_MANAGER_ROLE(), await feeManager.getAddress());

    await doveAdmin.connect(admin).grantRole(await doveAdmin.EMERGENCY_ADMIN_ROLE(), await emergencyAdmin.getAddress());
  ...
  async function bypassTimelockAndLaunch(doveAdmin: DOVEAdmin, admin: SignerWithAddress) {
  ...
  describe.skip('Basic Token Functionality', function () {
    it('Should set the correct initial state', async function () {
      const { doveToken, doveInfo, deployer, user1: _user1, charityWallet } = await loadFixture(deployTokenFixture);
    ...
    it('Should transfer tokens correctly after launch', async function () {
      const { doveToken, doveAdmin: _doveAdmin, admin: _admin, user1, user2 } = await loadFixture(deployTokenFixture);
    ...
    it('Should prevent transfers when paused', async function () {
      ...
      const { doveToken, doveAdmin, admin, user1 } = await loadFixture(deployPausedTokenFixture);
  ...
  describe.skip('Admin Functionality', function () {
    ...
    it('Should set DEX status correctly', async function () {
      const { doveToken, doveAdmin, doveInfo, dexRouter, admin } = await loadFixture(deployTokenFixture);
    ...
    it.skip('Should disable early sell tax (pending full ecosystem fixture)', async function () {
      const { doveToken, doveAdmin, doveInfo, emergencyAdmin, admin } = await loadFixture(deployTokenFixture);
    ...
    it.skip('Should disable max tx limit (pending contract fix)', async function () {
      const { doveToken, doveAdmin, doveInfo, admin, user1 } = await loadFixture(deployTokenFixture);
  ...
  describe.skip('Governance Functionality', function () {
    it('Should implement admin contract updates with multiple approvals', async function () {
      const { doveToken, doveAdmin, doveGovernance, admin, emergencyAdmin } = await loadFixture(deployTokenFixture);
      ...
      const proposeTx = await doveGovernance.connect(admin).proposeAdminUpdate(await newDOVEAdmin.getAddress());

      // Get the proposal ID - in ethers v6, we use a simple sequential ID
      const proposalId = 0;

      // Verify proposal was created properly
      const [newAdmin, , approvalCount, executed] = await doveGovernance.getAdminUpdateProposal(proposalId);
      ...
      const [, , approvals, isExecuted] = await doveGovernance.getAdminUpdateProposal(proposalId);
  ...
  describe('Security Patch Tests', function () {
    it('Only fee manager can call fee-moving functions', async function () {
      const { doveToken, feeManager: _feeManager, user1 } = await loadFixture(deploySimpleTokenFixture);
      ...
      await expect(
        doveToken.connect(user1).transferFeeFromContract(await doveToken.getAddress(), await user1.getAddress(), 1n)
