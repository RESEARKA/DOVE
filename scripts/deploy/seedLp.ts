...
async function seedLiquidity() {
  ...
  const ethAmount = ethers.parseEther('1.67'); // 1.67 ETH as specified in guidelines

  // Set DOVE amount for liquidity (10 billion)
  const doveAmount = ethers.parseEther('10000000000'); // 10B DOVE
  ...
  if (currentAllowance < doveAmount) {
    console.log(`Approving DEX Router to spend ${ethers.formatEther(doveAmount)} DOVE...`);
  ...
  console.log(
    `Adding liquidity: ${ethers.formatEther(doveAmount)} DOVE + ${ethers.formatEther(ethAmount)} ETH`
  );

  // Add liquidity with minimum amounts set to 99% to account for minor price fluctuations
  const minToken = (doveAmount * 99n) / 100n; // 99%
  const minEth = (ethAmount * 99n) / 100n;

  const tx = await router.addLiquidityETH(
    doveAddress, // token address
    doveAmount, // token amount
    minToken, // min token amount (99%)
    minEth, // min ETH amount (99%)
